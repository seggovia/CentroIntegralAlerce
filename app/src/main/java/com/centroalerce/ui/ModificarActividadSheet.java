...
public class ModificarActividadSheet extends BottomSheetDialogFragment {
    ...
    public static ModificarActividadSheet newInstance(@NonNull String actividadId){
        ...
        ModificarActividadSheet s = new ModificarActividadSheet();
        s.setArguments(b);
        return s;
    }
    ...
    public void onViewCreated(@NonNull View v, @Nullable Bundle b) {
        ...
        if (!permissionChecker.checkAndNotify(getContext(),
            ...
            android.util.Log.d("ModificarSheet", "🚫 Usuario sin permisos para modificar");
            dismiss();
            return;
        }

        android.util.Log.d("ModificarSheet", "✅ Usuario autorizado para modificar");
        ...
        // Estáticos
        actPeriodicidad.setAdapter(new ArrayAdapter<>(requireContext(),
                android.R.layout.simple_list_item_1, periodicidades));
        ...
        MaterialButton btnGuardar = v.findViewById(R.id.btnGuardarCambios);
        if (btnGuardar != null) {
    ...
    private void bind(DocumentSnapshot doc){
        ...
        selectByIdOrName(actLugar,
                firstNonEmpty(doc.getString("lugar_id"), doc.getString("lugarId"), doc.getString("lugar")),
                firstNonEmpty(doc.getString("lugarNombre"), doc.getString("lugar")));
        selectByIdOrName(actOferente,
                firstNonEmpty(doc.getString("oferente_id"), doc.getString("oferenteId"), doc.getString("oferente")),
                firstNonEmpty(doc.getString("oferenteNombre"), doc.getString("oferente")));
        selectByIdOrName(actSocio,
                firstNonEmpty(doc.getString("socio_id"), doc.getString("socioId"), doc.getString("socioComunitario")),
                firstNonEmpty(doc.getString("socio_nombre"), doc.getString("socioComunitario")));
        selectByIdOrName(actProyecto,
                firstNonEmpty(doc.getString("proyecto_id"), doc.getString("project_id"), doc.getString("proyecto")),
    ...
    private void guardar(){
        ...
        up.put("tipo", tipo);
        up.put("periodicidad", periodicidad.toUpperCase(java.util.Locale.ROOT));

        if (!TextUtils.isEmpty(cupoStr)) {
            try {
                up.put("cupo", Integer.parseInt(cupoStr));
        ...
        if (!TextUtils.isEmpty(oferente)) {
            up.put("oferente", oferente);
            up.put("oferentes", java.util.Arrays.asList(oferente));
            up.put("oferenteNombre", oferente);
        }

        if (!TextUtils.isEmpty(socio)) {
            up.put("socioComunitario", socio);
        }
        ...
        db.runTransaction(trx -> {
            ...
            android.util.Log.d("ModificarSheet", "✅ Cambios guardados por usuario: " + userId);
            toast("Cambios guardados");
            notifyChanged();
            dismiss();
        }).addOnFailureListener(e -> {
            android.util.Log.e("ModificarSheet", "❌ Error al guardar: " + e.getMessage());
    ...
    private void cargarColeccionAOptionsMulti(List<String> colecciones, AutoCompleteTextView view,
                                              @Nullable ArrayList<String> fallback){
        ...
        if (q != null && !q.isEmpty()){
            for (DocumentSnapshot d: q){
                ...
                if (!TextUtils.isEmpty(nombre)) {
                    xs.add(new OptionItem(id, nombre));
                }
            }
        }
        return xs;
    ...
    private void setDropText(AutoCompleteTextView v, String s){
        if (!TextUtils.isEmpty(s)) v.setText(s, false);
    }
        ...
        for (int i = 0; i < ad.getCount(); i++) {
            ...
            if (it instanceof OptionItem) {
                ...
                String on = java.text.Normalizer.normalize(oi.nombre, java.text.Normalizer.Form.NFD)
                        .replaceAll("\\p{M}", "").toLowerCase(java.util.Locale.ROOT);
                if (norm.equals(on) || norm.equals(oi.id.toLowerCase(java.util.Locale.ROOT))) {
                    return oi;
                }
            }
        }
    ...
    private String val(AutoCompleteTextView et){
        return et.getText() != null ? et.getText().toString().trim() : "";
    }
